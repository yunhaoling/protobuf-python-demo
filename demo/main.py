from generated_code import addressbook_pb2
...
def person_demo():
    ...
    assert deserialized_person.phones[0].type == addressbook_pb2.Person.HOME
...
def addressbook_demo():
    ...
    phone_john.type = addressbook_pb2.Person.HOME
    ...
    phone_alice.type = addressbook_pb2.Person.MOBILE

    serialized_data = address_book.SerializeToString()
    assert type(serialized_data) == bytes
    assert serialized_data == b'\n-\n\x08John Doe\x10\xd2\t\x1a\x10jdoe@example.com"\x0c\n\x08555-4321\x10\x01\n\x1c\n\tAlice Bob\x10\xae,"\x0c\n\x08666-8765\x10\x00'
    ...
    assert deserialized_john.phones[0].type == addressbook_pb2.Person.HOME
    ...
    assert deserialized_alice.phones[0].type == addressbook_pb2.Person.MOBILE
